...
mod tests {
    ...
    // End-to-end coverage through the `.ill` harness lives in
    // `examples/mqtt/`, which is exercised by `ill test examples/mqtt`. The
    // examples cover the same surface (connect, subscribe, qos 0/1/2,
    // receive, hex payload, capture/interpolate, disconnect, multi-actor
    // pub/sub, session takeover) without needing a custom broker image
