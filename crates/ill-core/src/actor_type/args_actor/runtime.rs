...
impl ArgsActorInstance {
    pub fn construct(
        ...
        for var in &args.vars {
            let value = match (cli_args.get(&var.name), &var.default) {
...
mod tests {
    ...
    fn args_with(
        ...
        cli: &[(&str, &str)],
    ) -> (ConstructArgs, BTreeMap<String, String>) {
        let cli_args = cli
            .iter()
    ...
    fn base_args(vars: Vec<DeclaredVar>) -> (ConstructArgs, BTreeMap<String, String>) {
        args_with(vars, &[])
    }
    ...
    fn missing_required_errors() {
        let (args, cli) = base_args(vec![var("required", None)]);
        let err = ArgsActorInstance::construct(&args, &cli).unwrap_err();
        assert!(
    ...
    fn required_uses_cli_override() {
        let (args, cli) = args_with(vec![var("required", None)], &[("required", "hello")]);
        let inst = ArgsActorInstance::construct(&args, &cli).unwrap();
        assert_eq!(
    ...
    fn default_used_when_no_override() {
        let (args, cli) = base_args(vec![var("opt", Some(Value::String("foo".into())))]);
        let inst = ArgsActorInstance::construct(&args, &cli).unwrap();
    ...
    fn number_default_coerces_cli_string() {
        let (args, cli) = args_with(vec![var("n", Some(Value::Number(7)))], &[("n", "42")]);
        let inst = ArgsActorInstance::construct(&args, &cli).unwrap();
    ...
    async fn self_view_exposes_members() {
        let (args, cli) = base_args(vec![var("x", Some(Value::Number(1)))]);
        let inst = ArgsActorInstance::construct(&args, &cli).unwrap();
    ...
    async fn check_command_is_ok() {
        let (args, cli) = base_args(vec![]);
        let mut inst = ArgsActorInstance::construct(&args, &cli).unwrap();
        let outcome = inst
            .execute(
                "check",
                &CommandArgs {
